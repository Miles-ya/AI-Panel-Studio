# AI Panel Studio MVP UI/UX 设计说明

> 文档状态：已实现布局基线｜v1.1｜2026-09-10
>
...
整体采用“浅色可信演播厅 + 高可读内容”的专业直播风格：暖白背景、雾蓝灰分层表面和明确留白传达可靠、可审阅的讨论现场；主持人与专家颜色只用于身份识别和实时活动提示，不应让彩色装饰压过内容。

| Token 类别 | 建议 | 用途 |
| -- | -- | -- |
| 画布 | `slate-50` / 雾蓝白 | 演播厅与应用背景 |
| 表面 | `white` / `slate-100` | 卡片、面板、浮层 |
| 边框 | `slate-200` | 分区、非激活卡片 |
| 主文字 | `slate-800` | 主题、正文 |
| 次文字 | `slate-500` | 时间、职业、说明 |
| 主操作 | `indigo-600` | Start、确认、重试 |
| 成功/在线 | `emerald-700` | 实时连接、完成 |
| 警告 | `amber-700` | 总结重试中 |
| 错误 | `rose-700` | 失败、无法加载 |

Participant 的 `color` 是身份色，必须保留其 API 值；仅用于头像边缘、姓名标识条、发言左侧色条和 speaking 环，不应用作大面积底色。所有文字仍须满足浅色背景下可读性，身份色不能是判断状态的唯一方式。
...
每张卡片包含：状态标签、主题、参与人数、最近更新时间，以及与状态对应的操作。整张卡片可点击并可通过 Enter/Space 键进入对应页面；原有文字操作保留为视觉提示，不额外改变首页信息结构。
...
**布局**：采用编辑式嘉宾名单。顶部以收紧的话题摘要、阵容计数与“确认后显式开始”的说明构成两栏；主持人卡片横跨完整内容宽度，成为节目主位；专家卡片使用带顺序编号的对称响应式网格。底部固定确认栏左侧显示阵容就绪信息，右侧包含“重新生成”和“确认阵容”。
...
当前桌面演播厅不是旧版三栏工作台：嘉宾首先以全视口宽的横向舞台轨道呈现，中央为约 1100px 演播厅内容列内的完整宽度 Transcript，下面依次放置共识/分歧和总结。Transcript 仍是最大、最高可用高度的独立阅读区域；嘉宾和 Insight 是帮助理解上下文的辅助信息，不能伪装成模型推理过程。

```text
┌──────────────────────────────────────────────────────────────────────────────┐
│ AI Panel Studio   [讨论主题 …]                                   [发起讨论] │
├──────────────────────────────────────────────────────────────────────────────┤
│ 直播中 / 已结束 · 公开发言进度 · 连接状态              [开始 / 停止讨论]    │
├──────────────────────────────────────────────────────────────────────────────┤
│  嘉宾舞台轨道（横向滚动）：主持人 → 专家 → 专家 → …                         │
│  头像 · 姓名 · Title · 公开状态；悬停可阅读公开立场/关注点                  │
├──────────────────────────────────────────────────────────────────────────────┤
│                         现场 Transcript（独立滚动）                          │
│    发言色条 · 姓名 · 职业 / Title · 正文 · 新发言提示                        │
├──────────────────────────────────────────────────────────────────────────────┤
│  已形成共识                                      │ 关键分歧                   │
├──────────────────────────────────────────────────────────────────────────────┤
│  主持人总结 / 总结生成中 / 降级与重试                                      │
...
#### 嘉宾舞台轨道（Participant Rail）

主持人固定在轨道首位，其余专家按 API participants 顺序横向排布。轨道可横向滚动；短阵容居中。每位嘉宾显示头像占位（姓名首字）、姓名、Title 与公开状态；桌面悬停卡片时显示公开立场或 `public_focus`，不把长文常驻在轨道中挤压舞台。`speaking` 以身份色和轻微位置变化强调，`preparing` 与 `idle` 仍以公开文字状态区分；动画必须支持 `prefers-reduced-motion` 降级为无动画状态。

`public_focus` 为空时显示“等待讨论推进”，不显示“思考中”、内部调度或推理过程。状态文字不能只依赖颜色或动画；当前实现通过桌面悬停显示详情，嘉宾卡不提供可点击的立场控制。
...
#### 共识、分歧与总结（Transcript 下方）

Transcript 下方是两列的“已形成共识”和“关键分歧”卡片；窄屏时堆叠。每项只显示 Insight `content`；不显示模型置信度、隐藏来源链或内部操作。空状态分别为“共识正在形成”“分歧尚未显现”。每次更新以当前完整活跃集合替换卡片内容。

再下方为独立的总结卡：运行中说明总结将在收束后生成，结束后显示自然语言总结；`pending` 显示生成中，`fallback` 显示降级说明和“重试总结”。这三个区域不影响 Transcript 的滚动位置与阅读状态。
...
| 视口 | 布局 | 优先级 |
| -- | -- | -- |
| 桌面 `≥701px` | 主题与控制 → 全视口宽嘉宾横向轨道 → 宽 Transcript → 双列共识/分歧 → 总结 | 让 Transcript 保持单一、宽阔的阅读主舞台 |
| 窄屏 `≤700px` | 相同主顺序；嘉宾轨道继续横向滚动，共识/分歧改为单列 | 先保证阅读发言，再提供辅助信息 |

> 设计决策：不为移动端另造功能或数据模型；只在窄屏把共识与分歧堆叠。嘉宾始终是横向舞台轨道，而不是挤压 Transcript 的桌面侧栏。

### 4.2 高度与滚动规则

- 应用壳使用视口高度（动态视口高度兼容移动浏览器）；顶栏和演播厅头部固定，正文获得剩余高度。
- **桌面**：Transcript 固定为受控高度的独立滚动阅读区；嘉宾轨道独立横向滚动。共识、分歧和总结位于 Transcript 下方，随内容自然延展，不能遮挡或重置 Transcript。
- **窄屏**：演播厅沿用同一信息顺序；Transcript 仍使用受控高度和内部滚动，嘉宾保持横向轨道，共识/分歧以单列展示。
- 两个 Insight 卡片内容较长时应可完整阅读，不能裁切最后一项；其更新不得改变 Transcript 滚动位置。
...
| `InsightPanel` | Transcript 下方的共识或分歧条目 | active Insights | 接收完整活跃集合，不无限追加 |
...
- 浅色主题下文字/背景对比度达到 WCAG AA；Participant 身份色若对比不足，仅作为装饰色条。
...
3. 演播厅视觉中心永远是 Transcript；嘉宾使用全宽横向舞台轨道，共识/分歧与总结位于其下，窄屏仅将 Insight 卡片堆叠而不压缩正文列。
...
### 8.1 浅色可信演播厅实现补充（v1.1）

本阶段根据开发者确认，将原先“深色演播厅”视觉表达替换为浅色专业观看环境，不改变页面、组件、数据字段、生命周期或交互。实现使用暖白/雾蓝灰画布、白色卡片、浅灰分隔与靛蓝主操作，营造可阅读、可审阅的可信感；实时性仅由文字状态、低强度在线点、身份色细边和克制动效表达。所有身份色仍不能单独承载状态含义。
