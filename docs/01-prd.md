...
5. 演播厅订阅本 Discussion 的 SSE；主持人开场，调度器依据上下文安排发言。
...
- **关键异常**：SSE 中断保留已收内容并重连；快照失败可重试。

### 专家实时状态

- **目的**：展示实时感，且不泄露隐藏推理。
- **输入**：`idle`、`preparing`、`speaking` 与 `public_focus`。
- **核心行为**：每位专家独立小窗显示状态和公开关注点；主持人亦可显示。
- **输出**：Participant 局部更新。
- **关键异常**：未知或跨 Discussion 事件丢弃并记录诊断。
...
- **输出**：有序发言流。
- **关键异常**：按 ID 去重、按 sequence 排序；未知发言人不得当作正常发言渲染。

### 实时共识

- **目的**：展示逐步形成的共同判断。
- **输入**：活跃 `consensus` Insight。
- **核心行为**：对同一语义观点更新既有项，不无限新增重复内容。
- **输出**：当前活跃共识。
